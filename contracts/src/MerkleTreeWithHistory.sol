...
library MerkleTreeWithHistoryLib {
    ...
    function initialize(MerkleTreeWithHistory storage _tree, IHasher _hasher, uint32 _levels) public {
        require(_levels > 0, "_levels should be greater than zero");
        require(_levels < 32, "_levels should be less than 32");

        _tree.hasher = _hasher;
        _tree.levels = _levels;
        _tree.currentRootIndex = 0;
        _tree.nextIndex = 0;

        for (uint32 i = 0; i < _levels; i++) {
        _tree.filledSubtrees[i] = zeros(i);
        }

        _tree.roots[0] = zeros(_levels - 1);
    }

    function insert(MerkleTreeWithHistory storage _tree, bytes32 _leaf) public returns (uint32 index) {
        IHasher hasher = _tree.hasher;
        uint32 levels = _tree.levels;
        uint32 currentRootIndex = _tree.currentRootIndex;
        uint32 nextIndex = _tree.nextIndex;

        require(nextIndex != uint32(2)**levels, "Merkle tree is full. No more leaves can be added");
        ...
        for (uint32 i = 0; i < levels; i++) {
            if (currentIndex % 2 == 0) {
                ...
                right = zeros(i);
                _tree.filledSubtrees[i] = currentLevelHash;
            } else {
                left = _tree.filledSubtrees[i];
        ...
        _tree.currentRootIndex = newRootIndex;
        _tree.roots[newRootIndex] = currentLevelHash;
        _tree.nextIndex = nextIndex + 1;
        return nextIndex;
    }
    ...
    */
    function isKnownRoot(MerkleTreeWithHistory storage _tree, bytes32 _root) public view returns (bool) {
        if (_root == 0) {
        return false;
        }
        uint32 currentRootIndex = _tree.currentRootIndex;
    ...
    */
    function getLastRoot(MerkleTreeWithHistory storage _tree) public view returns (bytes32) {
